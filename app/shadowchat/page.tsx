...
export default function JoinPage() {
  ...
  const canConnect =
    trimmedHandle.length >= 2 && isValidRoomCode(normalizedCode)

  const codeHint = useMemo(() => {
    if (normalizedCode.length === 0) {
      return 'Paste a code, or generate one'
    }

    if (normalizedCode.length < 8) {
      return `${8 - normalizedCode.length} more characters`
    }

    return isValidRoomCode(normalizedCode)
      ? 'Code looks valid'
  ...
  function handleConnect(event: React.FormEvent) {
    ...
    if (!isValidRoomCode(normalizedCode)) {
      setError(
        'Room code must be 8 characters (0-9, A-Z without I, L, O, U).',
      )
      return
    }
    ...
    <main className="sc-grid relative min-h-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1080px] flex-col px-6 py-7">
            ...
            <section className="sc-rise">
              ...
              <h1 className="mt-4 text-[34px] font-semibold leading-[1.12] tracking-tight sm:text-[42px]">
                ...
                <span className="text-[var(--color-muted)]">
                  Share the code. Talk. Vanish.
                </span>

                <span className="sc-caret ml-1 text-[var(--color-accent)]">
                  _
                </span>
              </h1>
            </section>
            ...
            <div className="flex flex-col gap-5">
              ...
              <section className="sc-card sc-rise p-5 sm:p-6">
                ...
                >
                  <label className="flex flex-col gap-2">
                    ...
                    >
                      2–24 characters. Not verified — handles are
                      cosmetic.
                    </span>
                  </label>
                  ...
                  <label className="flex flex-col gap-2">
                    ...
                    <span className="relative block">
                      <input
                        className="sc-field pr-[112px] font-[family-name:var(--font-mono)] tracking-[0.24em] uppercase"
                ...
                <p className="mt-4 border-t border-[var(--color-line)] pt-4 text-[11px] leading-5 text-[var(--color-faint)]">
                  Anyone holding the code can enter the room. Treat it
                  like a password and share it over a channel you
                  already trust.
                </p>
              </section>
              ...
              <dl className="grid gap-4">
                {FACTS.map((fact) => (
                  <div
                    key={fact.label}
                    className="sc-card p-3.5"
                  >
                    <dt className="sc-label">
                      {fact.label}
                    </dt>

                    <dd className="mt-1.5 text-[13px] leading-5 text-[var(--color-text)]">
                      {fact.value}
                    </dd>
                  </div>
                ))}
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--color-line)] pt-5 text-[11px] text-[var(--color-faint)]">
          <span>
            Zero Identity · Pure Communication
          </span>

          <span className="font-[family-name:var(--font-mono)]">
            40-bit room entropy · temporary storage, deleted on
            expiry
          </span>
        </footer>
      </div>
